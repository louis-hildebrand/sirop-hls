...
architecture tb of testbench is
    ...
    -- Easier debugging
    signal   data_t : signed(31 downto 0);
    ...
    pure function get_input_data (i : in integer; j : in integer) return signed is
    begin
        if ((i mod 20) < 10) = ((j mod 20) < 10) then
            return to_signed(255, 32);
        else
            return to_signed(0, 32);
        end if;
    end function;

    pure function get_output_data (i_top : in integer; j_left : in integer) return signed is
        variable gx           : signed(31 downto 0);
        variable gy           : signed(31 downto 0);
        variable norm_squared : signed(31 downto 0);
        variable norm         : signed(31 downto 0);
    ...
    begin
        ...
        for i_delta in 0 to 2 loop
            ...
            for j_delta in 0 to 2 loop
                ...
                if j_delta = 0 then
                    if i_delta = 1 then
                        gx := gx - resize(2*get_input_data(i, j), 32);
                    else
                        gx := gx - get_input_data(i, j);
                    end if;
                elsif j_delta = 2 then
                    if i_delta = 1 then
                        gx := gx + resize(2*get_input_data(i, j), 32);
                    else
                        gx := gx + get_input_data(i, j);
        ...
        for i_delta in 0 to 2 loop
            ...
            for j_delta in 0 to 2 loop
                ...
                if i_delta = 0 then
                    if j_delta = 1 then
                        gy := gy - resize(2*get_input_data(i, j), 32);
                    else
                        gy := gy - get_input_data(i, j);
                    end if;
                elsif i_delta = 2 then
                    if j_delta = 1 then
                        gy := gy + resize(2*get_input_data(i, j), 32);
                    else
                        gy := gy + get_input_data(i, j);
        ...
        norm_squared := resize(gx*gx + gy*gy, 32);
        norm := to_signed(integer(floor(
            sqrt(
                real(to_integer(norm_squared))
            )
        )), 32);
    ...
    begin
        ...
        for i_top in 0 to HEIGHT-3 loop
            for j_left in 0 to WIDTH-3 loop
                ...
                ready <= "111";
                assert(data = std_logic_vector(expected)) report "Wrong data at t = " & integer'image(t) & " (i=" & integer'image(i_top) & ",j=" & integer'image(j_left) & "): expected " & integer'image(to_integer(expected)) & ", got " & integer'image(to_integer(signed(data))) & ".";
            end loop;
        end loop;
