...
class StreamTests extends AnyFunSuite {
  ...
  test("StmCst:Int") {
    ...
    assert(ir.eval(s.tchk(Map())) == StmLiteral.ints(3, 3, 3, 3))
  }

  test("StmCst:Tuple") {
    val c = Tuple(False, 99)()
    val s = StmCst(3, c)()
    assert(ir.eval(s.tchk(Map())) == StmLiteral(c, c, c)())
  }

  test("StmCst:Stream") {
    val c = IntCst(42)
    val s = StmCst(2, StmCst(3, c)())()
    assert(ir.eval(s.tchk(Map())) == StmLiteral(c, c, c, c, c, c)())
  }

  test("StmCount") {
    val s = StmCount(5)()
    assert(ir.eval(s) == StmLiteral.ints(0, 1, 2, 3, 4))
    assert(ir.eval(s.tchk(Map())) == StmLiteral.ints(0, 1, 2, 3, 4))
  }

  test("StmCountFrom") {
    val s = StmCountFrom(3, 4)()
    assert(ir.eval(s) == StmLiteral.ints(3, 4, 5, 6))
    assert(ir.eval(s.tchk(Map())) == StmLiteral.ints(3, 4, 5, 6))
  }

  test("StmRange(4, 3, 2)") {
    val s = StmRange(4, 3, 2)()
    assert(ir.eval(s) == StmLiteral.ints(3, 5, 7, 9))
    assert(ir.eval(s.tchk(Map())) == StmLiteral.ints(3, 5, 7, 9))
  }

  test("StmRange(3, 2, -3)") {
    val s = StmRange(3, 2, -3)()
    assert(ir.eval(s) == StmLiteral.ints(2, -1, -4))
    assert(ir.eval(s.tchk(Map())) == StmLiteral.ints(2, -1, -4))
  }
  ...
  test("StmCst2D") {
    ...
    assert(ir.eval(s.tchk(Map())) == expected.flatten)
  }
  ...
  test("Iterate:Square") {
    ...
    val e = (nVal: Int) => Let(n, nVal, iter)().tchk(Map())
  ...
  test("Iterate:PlusTwo") {
    ...
    val iter = Iterate(n, Tuple(-10)(), Function(x, Tuple(x.__0 + 2)())())()
      .tchk(Map(n -> TyInt))
    val e =
      (nVal: Int) => Let(n, nVal, iter)().tchk(Map())
  ...
  test("StmAccess:3D") {
    ...
    assert(
      ir.eval(StmAccess(s, 0)().tchk(Map()))
        == StmLiteral(expected.head.flatten: _*)()
    )
    assert(
      ir.eval(StmAccess(s, 1)().tchk(Map()))
        == StmLiteral(expected(1).flatten: _*)()
    )
    assert(
      ir.eval(StmAccess(s, 2)().tchk(Map()))
  ...
  test("Vec2Stm:1D") {
    ...
    assert(ir.eval(Vec2Stm(v)().tchk(Map())) == StmLiteral.ints(1, 2, 3, 4, 5))
  }
  ...
  test("Vec2Stm:2D") {
    ...
    assert(ir.eval(s.tchk(Map())) == expected)
  }
  ...
  test("StmPrepend:1D") {
    val s = StmCount(3)()
    val actual = StmPrepend(s, 42)().tchk(Map())
  ...
  test("StmPrepend:2D") {
    ...
    val actual = StmPrepend(s0, s1)().tchk(Map())
    val expected = StmLiteral(
      Seq(
  ...
  test("StmAppend:1D") {
    val s = StmCount(3)()
    val actual = StmAppend(s, 42, stmShape = Seq(3)).tchk(Map())
  ...
  test("StmAppend:2D") {
    ...
    val actual = StmAppend(s0, s1, stmShape = Seq(3, 3)).tchk(Map())
    val expected = StmLiteral(
      Seq(
        Seq(Tuple(0, 0)(), Tuple(0, 1)(), Tuple(0, 2)()),
        Seq(Tuple(1, 0)(), Tuple(1, 1)(), Tuple(1, 2)()),
  ...
  test("StmConcat:1D") {
    val actual1 = StmConcat(StmCst(1, 77)(), StmCst(1, 77)())().tchk(Map())
    assert(ir.eval(actual1) == StmLiteral.ints(77, 77))
    val actual2 = StmConcat(StmCount(3)(), StmCst(4, 77)())().tchk(Map())
  ...
  test("StmConcat:2D") {
    ...
    val actual = StmConcat(s0, s1)().tchk(Map())
    val expected = Seq(
