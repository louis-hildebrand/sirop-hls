...
class StreamTests extends AnyFunSuite {
  ...
  test("Iterate:Square") {
    val x = Param("x")(U32)
    val n = Param("n")()
    val iter = Iterate(n, IntCst(3), Function(x, x * x)())()
    val e = (nVal: Int) => Let(n, IntCst(nVal), iter)().tchk()
  ...
  test("Iterate:PlusTwo") {
    ...
    val n = Param("n")(U8)
    val iter =
      Iterate(n, Tuple(IntCst(-10))(), Function(x, Tuple(x.__0 + 2)())())()
    val e =
      (nVal: Int) => Let(n, IntCst(nVal), iter)().tchk()
  ...
  test("StmMap:1D-1D:+7") {
    val s = StmMap(StmCount(IntCst(5))(), U8 ::+ (x => x + 7))().tchk()
    assert(ir.eval(s) == StmLiteral(7, 8, 9, 10, 11)())
  }

  test("StmMap:1D-1D:Identity") {
    val s = StmMap(StmCount(IntCst(3))(), U8 ::+ (x => x))()
  ...
  test("StmMap:1D-1D:StmSum(StmCount(i))") {
    ...
    val s = StmMap(
      StmCount(IntCst(5))(),
      U8 ::+ (i => StmFold(StmCount(i + 1)(), C(0), PlusFunction(U8))())
    )().tchk()
    val expected =
  ...
  test("StmMap:1D-2D:StmCst") {
    val s =
      StmMap(StmCount(IntCst(4))(), U8 ::+ (c => StmCst(3, c)()))().tchk()
  ...
  test("StmMap:1D-2D:DiscardInputReturnStmCount") {
    val s =
      StmMap(StmCount(IntCst(4))(), U8 ::+ (_ => StmCount(3)()))().tchk()
    val expected = StmLiteral.ints(
      Seq(
        Seq(0, 1, 2),
        Seq(0, 1, 2),
  ...
  test("StmMap:2D-2D:StmScanInclusive") {
    ...
    val input = StmMap(
      StmCount(C(3))(),
      U8 ::+ (i => StmMap(StmCount(C(4))(), U8 ::+ (j => 10 * i + j))())
    )()
    val s = StmMap(
      input,
      TyStm(U8, 4) ::+ (row => StmScanInclusive(row, C(0), PlusFunction(U8))())
    )().tchk()
    val expected = StmLiteral.ints(
      Seq(
        Seq(0, 1, 3, 6),
  ...
  test("StmMap:2D-2D:StmScanExclusive") {
    ...
    val s = StmMap(
      input,
      TyStm(U8, 4) ::+ (row => StmScanExclusive(row, C(0), PlusFunction(U8))())
    )().tchk()
    val expected = StmLiteral.ints(
      Seq(
        Seq(0, 0, 1, 3),
  ...
  test("StmMap:2D-2D:StmPrepend") {
    val s = StmMap(
      StmCst2D(4, 5, C(42))(),
      TyStm(U8, 5) ::+ (s => StmPrepend(s, C(43))())
    )().tchk()
    val expected = StmLiteral.ints(
      Seq(
  ...
  test("StmMap:2D-2D:StmShiftLeft") {
    val s = StmMap(
      StmCount2D(C(2), C(3))(),
      TyStm((U8, U8), 3) ::+ (s => StmShiftLeft(s, Tuple(C(100), C(101))())())
    )().tchk()
    val expected = StmLiteral(
      Seq(
        Seq(
  ...
  test("StmMap:2D-2D:StmShiftRight") {
    val s = StmMap(
      StmCount2D(C(4), C(5))(),
      TyStm((U8, U8), 5) ::+ (s => StmShiftRight(s, Tuple(C(100), C(101))())())
    )().tchk()
    val expected = StmLiteral(
      Seq(
        Seq(
  ...
  test("StmMap:2D-2D:StmConcatBefore") {
    val s = StmMap(
      StmCst2D(5, 5, C(99))(),
      TyStm(U8, 5) ::+ (s => StmConcat(StmCount(C(3))(), s)())
    )().tchk()
    val expected = StmLiteral.ints(
      Seq(
  ...
  test("StmMap:2D-2D:StmConcatAfter") {
    val s = StmMap(
      StmCst2D(5, 5, C(99))(),
      TyStm(U8, 5) ::+ (s => StmConcat(s, StmCount(C(3))())())
    )().tchk()
    val expected = StmLiteral.ints(
      Seq(
    ...
    val s = StmMap(
      ...
      U8 ::+ (i =>
        StmMap(
          StmCount(C(3))(),
          U8 ::+ (j =>
            StmMap(StmCount(C(4))(), U8 ::+ (k => Tuple(i, j, k)()))()
          )
        )()
      )
    )().tchk()
  ...
  test("StmMap:RepeatedData1") {
    ...
    val b = StmRange(m, C(-1), C(2))().tchk().lower()
    val expected = {
  ...
  test("StmMap:RepeatedData2") {
    ...
    val subs: Map[Expr, Expr] = Map(
      a -> StmCount2D(C(n), C(k))().tchk().lower(),
      b -> StmCount2D(C(m), C(k))().tchk().lower()
    )
    val expected = {
  ...
  test("StmFold:RepeatedData") {
        ...
        TyStm(U16, n) ::+ (b =>
          StmMap(
            ...
            TyStm(U16, n) ::+ (rowA => {
              ...
              val dotProd = StmFold(products, C(0), PlusFunction(U16))()
              dotProd
            })
          )()
        )
    ...
    val b = StmRange(n, C(10), C(3))().tchk().lower()
    val expected = {
  ...
  test("StmAccess:1D") {
    val n = 3
    val i = Param("i")(U8)
    val s = StmRange(n, C(5), C(2))()
    val access = StmAccess(s, i)().tchk().lower()

    assert(ir.eval(Let(i, C(0), access)()) == StmLiteral(5)())
    assert(ir.eval(Let(i, C(1), access)()) == StmLiteral(7)())
    assert(ir.eval(Let(i, C(2), access)()) == StmLiteral(9)())
  }
  ...
  test("StmAccess:2D") {
    ...
    assert(ir.eval(Let(i, C(0), access)()) == row(0))
    assert(ir.eval(Let(i, C(1), access)()) == row(1))
    assert(ir.eval(Let(i, C(2), access)()) == row(2))
    assert(ir.eval(Let(i, C(3), access)()) == row(3))
  }
  ...
  test("StmFold:1D:Sum") {
    val sum =
      StmFold(StmCount(C(6))(), C(3), PlusFunction(U8))()
        .tchk()
  ...
  test("StmFold:1D:Product") {
    val prod =
      StmFold(StmRange(5, C(1), C(1))(), C(1), TimesFunction(U8))()
        .tchk()
  ...
  test("StmFold:1D:HornerMethod") {
    ...
    val coefficients = StmRange(5, C(2), C(1))()
    val x = 2
    val y = StmFold(
      coefficients,
      C(0),
  ...
  test("StmFold:2D:SumWide") {
    ...
    val s = StmMap(
      StmCount(C(3))(),
      U8 ::+ (i => StmMap(StmCount(C(5))(), U8 ::+ (j => i + j))())
    )()
    val sum = StmFold(
      s,
      C(0),
  ...
  test("StmFold:2D:SumNarrow") {
    ...
    val s = StmMap(
      StmCount(C(4))(),
      U8 ::+ (i => StmMap(StmCount(C(2))(), U8 ::+ (j => i + j))())
    )()
    val sum = StmFold(
      s,
      C(0),
  ...
  test("StmFold:2D:SumColumn") {
    ...
    val s = StmMap(
      StmCount(C(4))(),
      U8 ::+ (i => StmMap(StmCount(C(4))(), U8 ::+ (j => i + j + 1))())
    )()
    val x = StmFold(
      s,
      C(13),
  ...
  test("StmFold:2D:Product") {
    ...
    val s = StmMap(
      StmCount(C(3))(),
      U8 ::+ (i => StmMap(StmCount(C(3))(), U8 ::+ (j => i + j + 1))())
    )()
    val prod = StmFold(
      s,
      C(1),
  ...
  test("StmFold:3D:Sum") {
    ...
    val s = StmMap(
      ...
      U8 ::+ (i =>
        StmMap(
          StmCount(C(2))(),
          U8 ::+ (j => StmMap(StmCount(C(4))(), U8 ::+ (k => i + j + k))())
        )()
      )
    )()
    val sum = StmFold(
  ...
  test("StmFold:3D:Product") {
    ...
    val s = StmMap(
      ...
      U8 ::+ (i =>
        StmMap(
          StmCount(C(2))(),
          U8 ::+ (j => StmMap(StmCount(C(2))(), U8 ::+ (k => i + j + k + 1))())
        )()
      )
    )()
    val prod = StmFold(
  ...
  test("StmScanInclusive:1D:Sum") {
    // [2, 3,  4,  5,  6]
    val s = StmMap(StmCount(C(5))(), U8 ::+ (x => x + 2))()
    // [2, 7, 18, 41, 88]
    val sum =
      StmScanInclusive(s, C(0), U8 ::+ (acc => U8 ::+ (x => x + 2 * acc)))()
        .tchk()
  ...
  test("Vec2Stm:2D") {
    val v =
      VecBuild(
        C(4),
        U32 ::+ (i => VecBuild(C(3), U32 ::+ (j => Tuple(i, j)()))())
      )()
    val s = StmMap(
      Vec2Stm(v)(),
      TyVec((U32, U32), C(3)) ::+ (v => Vec2Stm(v)())
    )().tchk()
  ...
  test("Vec2Stm:Vec[Stm[Int]]") {
    ...
    val m = 3
    val vs =
      VecBuild(C(n), U32 ::+ (i => StmRange(C(m), C(42), i)()))()
  ...
  test("Stm2Vec2Stm") {
    val s = Param("s")(TyStm(U32, C(6)))
    val actual =
      StmMap(Stm2Vec(s)(), TyVec(U32, C(6)) ::+ (v => Vec2Stm(v)()))()

    val s0 = StmCount(C(6))()
    val expected0 = StmLiteral(0, 1, 2, 3, 4, 5)()
    assert(ir.eval(Let(s, s0, actual)().tchk()) == expected0)

    val s1 = StmCst(6, C(42))()
  ...
  test("StmConcat:1D") {
    val u7 = TyUInt(7)
    val s1 = StmCst(4, C(-77))()
    val s2 = StmMap(StmCount(C(2))(), u7 ::+ (x => ReshapeData(x, I8)()))()
    val s3 = StmMap(StmCount(C(3))(), u7 ::+ (x => ReshapeData(x, I8)()))()

    val actual1 = StmConcat(s1, s1)().tchk().lower()
    val expected1 = StmLiteral((0 until 8).map(_ => C(-77)): _*)()
  ...
  test("StmConcat:2D") {
    val s0 = StmCst2D(2, 2, Tuple(C(99), C(99))())()
    val s1 = StmCount2D(C(3), C(2))()
  ...
  test("StmConcat:3D") {
    val s0 =
      StmMap(StmCount(C(3))(), U8 ::+ (_ => StmCst2D(2, 2, C(100))()))()
    val s1 = StmMap(StmCount(C(2))(), U8 ::+ (i => StmCst2D(2, 2, i)()))()
    val actual = StmConcat(s0, s1)().tchk()
    val expected = StmLiteral.ints(
      Seq(
        Seq(
  ...
  test("StmPrepend:1D") {
    val s = StmCount(C(3))()
    val actual = StmPrepend(s, C(42))().tchk()
    assert(ir.eval(actual) == StmLiteral(42, 0, 1, 2)())
  }

  test("StmPrepend:2D") {
    val s0 = StmCount2D(C(3), C(3))()
    val s1 = StmCst(3, Tuple(C(42), C(42))())()
  ...
  test("StmPrepend:3D") {
    val s0 =
      StmMap(StmCount(C(2))(), U8 ::+ (_ => StmCst2D(2, 2, C(42))()))()
    val s1 = StmCst2D(2, 2, C(99))()
  ...
  test("StmAppend:1D") {
    val s = StmCount(C(3))()
    val actual = StmAppend(s, C(42))().tchk()
  ...
  test("StmPrefix:1D") {
    ...
    assert(ir.eval(Let(k, C(0), prefix)()) == StmLiteral()())
    assert(ir.eval(Let(k, C(1), prefix)()) == StmLiteral(0)())
    assert(ir.eval(Let(k, C(2), prefix)()) == StmLiteral(0, 1)())
    assert(ir.eval(Let(k, C(3), prefix)()) == StmLiteral(0, 1, 2)())
  }

  test("StmPrefix:2D") {
    val s = StmCount2D(C(2), C(3))()
    val k = Param("k")(U8)
    val prefix = StmPrefix(s, k)().tchk()
    ...
    assert(ir.eval(Let(k, C(0), prefix)()) == expected(0))
    assert(ir.eval(Let(k, C(1), prefix)()) == expected(1))
    assert(ir.eval(Let(k, C(2), prefix)()) == expected(2))
  }

  test("StmPrefix:3D") {
    val s = StmMap(StmCount(C(2))(), U8 ::+ (_ => StmCount2D(4, 5)()))()
  ...
  test("StmSuffix:1D") {
    ...
    assert(ir.eval(Let(k, C(0), suffix)()) == StmLiteral()())
    assert(ir.eval(Let(k, C(1), suffix)()) == StmLiteral(2)())
    assert(ir.eval(Let(k, C(2), suffix)()) == StmLiteral(1, 2)())
    assert(ir.eval(Let(k, C(3), suffix)()) == StmLiteral(0, 1, 2)())
  }

  test("StmSuffix:2D") {
    val s = StmCount2D(C(3), C(2))()
    val k = Param("k")(U8)
    val suffix = StmSuffix(s, k)().tchk()
    ...
    assert(ir.eval(Let(k, C(0), suffix)()) == expected(0))
    assert(ir.eval(Let(k, C(1), suffix)()) == expected(1))
    assert(ir.eval(Let(k, C(2), suffix)()) == expected(2))
    assert(ir.eval(Let(k, C(3), suffix)()) == expected(3))
  }
  ...
  test("StmShiftLeft:1D") {
    val s = StmShiftLeft(StmCount(C(3))(), C(42))().tchk()
    assert(ir.eval(s) == StmLiteral(1, 2, 42)())
  }

  test("StmShiftLeft:2D") {
    val s0 = StmCount2D(C(3), C(4))()
    val s1 = StmMap(StmCount(C(4))(), U8 ::+ (j => Tuple(C(99), j)()))()
  ...
  test("StmShiftLeft:3D") {
    val s0 = StmMap(StmCount(C(2))(), U8 ::+ (i => StmCst2D(4, 5, i)()))()
    val s1 = StmCst2D(4, 5, C(42))()
  ...
  test("StmShiftRight:1D") {
    val s = StmShiftRight(StmCount(C(3))(), C(42))().tchk()
  ...
  test("StmRepeat:3D") {
    val s = StmMap(StmCount(C(2))(), U8 ::+ (i => StmCst2D(3, 4, i)()))()
    val expected = Seq(
      Seq(
        Seq(0, 0, 0, 0),
        Seq(0, 0, 0, 0),
  ...
  test("StmReverse:1D") {
    val s = StmReverse(StmCount(C(5))())().tchk()
    val expected = StmLiteral(4, 3, 2, 1, 0)()
    assert(ir.eval(s) == expected)
  }

  test("StmReverse:2D") {
    val s = StmReverse(StmCount2D(C(3), C(3))())().tchk()
    val expected = StmLiteral(
      Tuple(2, 0)(),
      Tuple(2, 1)(),
      Tuple(2, 2)(),
  ...
  test("StmJoin:3D-2D-1D") {
    ...
    //   [(1, 0), (1, 1), (1, 2), (1, 3)],
    //   [(2, 0), (2, 1), (2, 2), (2, 3)]]]
    val s = StmMap(StmCount(C(2))(), U8 ::+ (_ => StmCount2D(3, 4)()))()
    val expected = StmLiteral(
      Seq(
        Seq(Tuple(0, 0)(), Tuple(0, 1)(), Tuple(0, 2)(), Tuple(0, 3)()),
        Seq(Tuple(1, 0)(), Tuple(1, 1)(), Tuple(1, 2)(), Tuple(1, 3)()),
  ...
  test("StmSlideS:1D:SameSize") {
    val actual = StmSlideS(StmCount(C(5))(), 5)().tchk()
  ...
  test("StmTranspose") {
    ...
    val actual =
      Let(
        n,
        2,
        Let(m, 2, Let(s, StmCount2D(C(2), C(2))(), transposed)())()
      )().tchk()
    assert(ir.eval(actual) == expected)
  }
