...
function print_basic_usage {
    echo "Usage: test_verilog.sh PROJ_DIR [-h|--help] [-v|--verbose] [-i|--interactive] [--time-limit=TL]"
}
...
function run_simulation {
    if [[ "$interactive_mode" == 'true' ]]; then
        ...
        timeout "$time_limit" vsim -c -do "$tcl_script" -t "$TIME_RESOLUTION" -L work -voptargs="+acc" Test
