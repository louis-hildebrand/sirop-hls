    ...
    e match {
      ...
      case p: Param =>
        substitutions.get(p) match {
          case Some(v) => v
          case None =>
            throw new RuntimeException("Param not found in substititions map")
        }
      ...
      case Add(e1: Expr, e2: Expr) =>
        eval(e1).asInstanceOf[IntCst].i + eval(e2).asInstanceOf[IntCst].i
      ...
      case False => False
      case IfThenElse(cond: Expr, trueE: Expr, falseE: Expr) =>
        if (eval(cond).asInstanceOf[BoolExpr]) eval(trueE) else eval(falseE)
      case NotEqual(e1: Expr, e2: Expr) =>
        eval(e1).asInstanceOf[IntCst].i != eval(e2).asInstanceOf[IntCst].i
      ...
      case StmNext(
            e: Expr
          ) => // need to ensure that from now on, cannot access e: can either make Next primitive take a new body for what to do next a bit like a function
        val stream: StmBuild = eval(e).asInstanceOf[StmBuild]
        assert(eval(stream.length).asInstanceOf[IntCst].i > 0)

        val next: Tuple =
          eval(FunCall(stream.nextF, stream.seed)).asInstanceOf[Tuple]
        ...
        Tuple(
          StmBuild(
            eval(stream.length + -1),
            eval(nextSeed),
            eval(stream.nextF)
              .asInstanceOf[Function] /*this function may have free parameters*/
          ),
          eval(nextElem)
        )
      ...
      case VecAccess(vec: Expr, i: Expr) =>
        eval(FunCall(eval(vec).asInstanceOf[VecBuild].f, eval(i)))
